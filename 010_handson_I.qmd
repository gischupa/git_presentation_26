---
format: revealjs
---

## Hands On I {#handson_I}

::: fragment
Anlegen des Repos `versuch_1`
:::

::: columns
::: column
::: fragment
[Variante 1]{.smaller}
``` bash
mkdir versuch_1
cd versuch_1
git init
git branch -m main
```
:::
:::

::: column
::: fragment
[Variante 2]{.smaller}
``` bash
git init versuch_1
cd versuch_1
git branch -m main
```
:::
:::
:::

:::{.notes}
* Wir bleiben im Ordner gitkurs  
* selbst versuchen lassen
* Alternative Varianten 
:::

---

Kontrolle

``` bash
git status 

# Ausgabe 
Auf Branch main
Noch keine Commits
nichts zu committen (erstellen/kopieren Sie Dateien und benutzen
Sie "git add" zum Versionieren)
```
::: {.fragment}
Inhalte erstellen

```{.bash code-line-numbers="1-3|5-7"}
echo "Version 1" > datei1.txt
git add datei1.txt
git commit -m "Erste Datei erstellt"

echo "Version 2" >> datei1.txt
git add datei1.txt
git commit -m "Erläuterung"
```
:::


:::{.notes}
ZIEL: längeres Log sehen

* Status-Befehl
* TIPPS zum weiteren Vorgehen im Text
* datei erstellen, füllen, adden, committen
* Datei erweitern , add, commit   
:::

---

Check

``` bash
git log

# Ausgabe
commit 6db4f3383512a7f3aaaf3b050334d7fd2c474688 (HEAD -> main)
Author: Wolfgang <hoeferwolf@t-online.de>
Date:
Thu Nov 13 10:46:50 2025 +0100
Erläuterung
commit 430149c83906005ae2c53c8eaf9ce230e3456ad0
Author: Wolfgang <hoeferwolf@t-online.de>
Date:
Thu Nov 13 10:46:50 2025 +0100
Erste Datei erstellt
```

---

Kontrolle

::: {.fragment}
``` bash
# Eingabe
git status

# Ausgabe 
Auf Branch main
nichts zu committen, Arbeitsverzeichnis unverändert
```
:::

::: {.fragment}
Weiterer Inhalt

``` bash
# Eingabe (Lasse ich zukünftig weg)
echo "Version 3" >> datei1.txt
echo "Version 1" >> datei2.txt

# Status anzeigen lassen
git status
```
:::


:::{.notes}
Status zuerst ohne Änderung,   
Dann eine neue Datei und Änderung an alter.  
Was bekommen wir?
:::

---

Ausgabe

``` bash
Auf Branch main 
Änderungen, die nicht zum Commit vorgemerkt sind:
(benutzen Sie "git add <Datei>...", um die Änderungen zum Commit vorzumerken)
(benutzen Sie "git restore <Datei>...", um die Änderungen im Arbeitsverzeichnis zu verwerfen)

    geändert: datei1.txt

Unversionierte Dateien: (benutzen Sie "git add <Datei>...", um die Änderungen zum Commit vorzumerken)

    datei2.txt

keine Änderungen zum Commit vorgemerkt (benutzen Sie "git add" und/oder "git commit -a")
```

:::{.notes}
Geändert / unversioniert
:::

---

Versionieren

``` bash
# Dateien aufnehmen
git add .

# Status abzeigen
git status

# Ausgabe
Auf Branch main
Zum Commit vorgemerkte Änderungen: (benutzen Sie "git restore --staged <Datei>..."
zum Entfernen aus der Staging-Area) 
    geändert:    datei1.txt
    neue Datei:  datei2.txt
```

:::{.notes}
Wir adden - was passiert?
Workdir sauber, stage erweitert
:::


---

Weitere Zeilen

``` bash
echo "Weitere Zeile" >> datei1.txt
echo "Weitere Zeile" >> datei2.txt
git status
```

---

Ausgabe

::: {.fragment}
``` bash
Auf Branch main
Zum Commit vorgemerkte Änderungen: (benutzen Sie "git restore --staged <Datei>..."
zum Entfernen aus der Staging-Area)
  geändert:    datei1.txt
  neue Datei:  datei2.txt
########## Ende Stage 
```
:::

::: {.fragment}
``` bash
####### Anfang Workdir
Änderungen, die nicht zum Commit vorgemerkt sind:
(benutzen Sie "git add <Datei>...", um die Änderungen zum Commit vorzumerken)
(benutzen Sie "git restore <Datei>...", um die Änderungen im Arbeitsverzeichnis zu verwerfen)
     geändert:    datei1.txt
     geändert:    datei2.txt
```
:::


:::{.notes}
Bestand wieder ändern. Was bekommen wir?  
Work / stage / commit verschiedene Versionen
:::


---

Commit des aktuellen Stands

::: {.fragment}
``` bash
git commit -m "nur Stage"
```

Aktueller Status
:::

::: {.fragment}
``` bash
git Status

# Ausgabe
Auf Branch main
Änderungen, die nicht zum Commit vorgemerkt sind:(benutzen Sie "git add <Datei>...", um die Änderungen zum Commitvorzumerken)
(benutzen Sie "git restore <Datei>...", um die Änderungen im Arbeitsverzeichnis zu verwerfen)
geändert:    datei1.txt
geändert:    datei2.txt

keine Änderungen zum Commit vorgemerkt (benutzen Sie "git add" und/oder "git commit -a")
```

:::

:::{.notes}
Was passiert beim Commit?  
Commit ändert das Workdir nicht, stage ist "leer"
:::

------------------------------------------------------------------------

Jetzt der Rest ...

``` bash
# Zum Stage hinzufügen
git add .
git status

# Ausgabe
Auf Branch main
Zum Commit vorgemerkte Änderungen:
(benutzen Sie "git restore --staged <Datei>..." zum Entfernen aus
der Staging-Area)
    geändert:    datei1.txt
    geändert:    datei2.txt
```

:::{.notes}
Wieder adden -> ?
Stage ist voll
nächster Commit möglich
:::