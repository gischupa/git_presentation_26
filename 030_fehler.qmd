...
* Einfache Fehler: Dateien restaurieren
* Größere Fehler: Zurücksetzen auf Stände

## Einfache Fehler

[Dateien restaurieren]{.smaller}
...
[Rücksetzen auf frühere Stände -- Varianten]{.smaller}
...
<div class="smaller">Probleme im Team, wegen Änderung der History, falls der 
aktuelle Stand bereits veröffentlicht war.</div>
:::
::: {.fragment}
Mixed (Quasi Standard)

```bash
git reset --soft <commit-hash>  # Workdir bleibt = Stage
...
```bash
git reset --mixed <commit-hash>  # Workdir bleibt, Stage vom Commit 
```
:::
...
::: {.notes}
Hinweis auf Scripte im Material
:::
...
[Warnung über *detached Head*]{.smaller}
:::
